...
*备注：下表中，✔️代表六面均输出信号，❌代表六面均不输出信号*。

| 电源 | 输出 | 输出方式 | 是否为毗邻的红石元件提供信号 | 是否强充能毗邻的红石导体 | 备注 | 示例图 |
| --- | :---: | --- | :---: | :---: | --- | :---: |
| 红石块 | **15** | 无条件输出 | ✔️ | ❌ | *是在命令方块系统中应用最多的一种信号源* | ![redstone_block](./img/section1/redstone_block.png) |
| 拉杆 | **15** | 拉下后提供信号，再拉回来则停止提供信号 | ✔️ | 仅附着方块 | *有些类似于现实世界的电源和单刀单掷开关的合体* | ![lever](./img/section1/lever.png) |
| 按钮 | **15** | 按下后的短暂时间间隔内能够提供信号，随后自身回弹并停止提供信号 | ✔️ | 仅附着方块 | · 分为木质按钮和石质按钮<br/>· 石质按钮提供信号的时间短（1 秒），木质按钮提供信号的时间长（1.5 秒）<br/>· 箭和三叉戟在射中木质按钮后还能提供更持久的信号（长达 1 分钟） | ![button](./img/section1/button.png)<br/>箭射中木质按钮后会持续激活木质按钮 |
| 压力板 | · **15**（木质/石质）<br/>· **由实体数决定**（轻质测重/重质测重） | 实体踩踏压力板后提供信号 | ✔️ | 仅下方 | · 分为 4 种：木质、石质、轻质测重（金）和重质测重（铁）<br/>· **石质压力板只在生物踩踏后激活，其他实体踩踏则不激活**<br/>· 轻质测重压力板每 1 个实体踩踏输出 1 信号<br/>· 重质测重压力板每 10 个实体踩踏输出 1 信号<br/> | ![pressure_plate_1](./img/section1/pressure_plate_1.png)<br/>1 个盔甲架踩踏 4 种压力板的表现<br/>![pressure_plate_2](./img/section1/pressure_plate_2.png)<br/>15 根箭（非生物）踩踏 4 种压力板的表现 |
| 侦测器 | **15** | 在检测（脸）端检查到方块有变化的时候，在输出端输出瞬时信号<br/> | 仅输出端 | 仅输出端 | 瞬时信号时长为 0.1 秒 | ![observer](./img/section1/observer.png)<br/>一种高频输出信号的简易装置，即两个侦测器的脸部正对<br/> |
| 陷阱箱 | **开箱人数** | 箱子打开时输出信号 | ✔️ | 仅下方 | *常常用于下方埋 TNT 或者隔一个方块埋 TNT 做陷阱* | ![trapped_chest](./img/section1/trapped_chest.png) |
| 绊线钩 | **15** | 1. 当实体经过绊线时输出信号，离开后则过段时间恢复<br/>2. 当绊线被用非剪刀破坏时，输出瞬时信号 | ✔️ | 仅附着方块 | 瞬时信号时长为 0.5 秒 | ![tripwire_hook](./img/section1/tripwire_hook.png) |
| 讲台 | **15** | 翻页时输出瞬时信号 | ✔️ | 仅下方 | 瞬时信号时长为 0.1 秒 | ![lectern](./img/section1/lectern.png) |
| 标靶 | **由击中位置决定** | 被弹射物击中时输出瞬时信号 | ✔️ | 仅下方 | · 瞬时信号时长由弹射物类型决定，箭和三叉戟为 1 秒，其余为 0.4 秒<br/>· 击中位置越靠近中心，信号强度越大<br/>· 常常用于改变红石线路走向的红石导体 | ![target_1](./img/section1/target_1.png)<br/>击中了标靶的三叉戟，但因为是瞬时信号，所以此时已不输出<br/>![target_2](./img/section1/target_2.png)<br/>改变线路走向 |
| 探测铁轨 | **15** | 有矿车经过时提供信号 | ✔️ | 仅下方 | | ![detector_rail](./img/section1/detector_rail.png) |

:::tip[实验 3.1-4]

请通过上表所示的图片示例，分别搭建对应的电路，体验一下各个电源的用法。

:::

## 红石刻

在上面的表格中，我们看到很多能够输出瞬时信号的元件。你发现了吗——这些瞬时信号的最短值是 0.1 秒，并且所有这些信号的时长都是 0.1 秒的倍数。

事实上，在红石系统领域，存在一个和游戏刻同等重要的类似概念，**红石系统的响应速度都是以 0.1 秒为单位的，一切的红石元件的响应时长都是 0.1 秒的倍数**，我们称这个时间为**1 个游戏刻（Redstone Tick）**。显然地，和以前的游戏刻的概念结合起来看，我们就能得到**1 秒= 10 红石刻（Redstone Tick, RT）= 20 游戏刻（Game Tick, GT）**，或**0.1 秒= 2 游戏刻= 1 红石刻**。这是时间转换原理的完全体，是极其重要的。

## 各种传输元件及其性质

传输元件的类型虽然相对较少，但是它们的特性却一个比一个复杂，用法一个比一个多变。因此，对于传输元件我们就不再使用表格概括，而是直接依次详细讲解。

:::tip[实验 3.1-5]

在传输元件这一部分的讲解中，请读者按照图片所示的电路在游戏中搭建出来，并体验电路的效果。
...
对于红石粉的降落，这里基岩版和 Java 版则存在一些差异，这里我们只谈基岩版。红石粉的降落必须要保证其侧面不能是中断的。红石导体自不必多说，对于红石绝缘体来说，常用的侧面不中断的方块是玻璃。读者也可以在下面这张图中看到信号爬升和信号降落之间的区别。
...
例如，在上图左 1 中，我们看到红石火把所附着的红石灯并没有亮，而其上方和侧边的红石灯却亮了起来；左 2 和左 3 则表明，不论如何摆放红石火把，都是强充能其上方的红石导体，而侧面的红石导体则不会被充能。

红石火把作为传输元件的基本特性在于**充能熄灭**。**如果其所附着的红石导体被充能，那么红石火把就会熄灭**。

![redstone_torch_3](./img/section1/redstone_torch_3.png)

这个特性是极其有意义的，它已延伸出诸多有用的特性，例如信号中续，利用两个红石火把延续信号；竖直传递信号，尤其是高度差较高的时候；以及取逻辑非，等等情况。

![redstone_torch_4](./img/section1/redstone_torch_4.png)

读者应当注意一个很关键的问题：**红石火把熄灭与重燃的响应时间是 0.1 秒**。例如在上图的中继器的设计中，拉下拉杆后需要 0.2 秒才能点亮红石灯；而上图的竖直爬升的设计则需要 0.4 秒才能点亮红石灯。这在红石系统领域中有一个专门的领域叫做**延迟（Delay）**，尤其在该领域中经常以红石刻为单位，一般说刻都是红石刻，例如上面两个例子就分别是 2 刻的延迟和 4 刻的延迟。*本教程只在本节（3.1）中把刻简称为红石刻，其他章节说刻都是游戏刻*，请读者务必注意区分。
