...
可以看到，这条命令和`/give`有很多相似处。例如，如果我们要给予 Steve 一个苹果，再移除 Steve 的苹果，可以写为
...
例如，调整 Alex 为生存模式，就可以用`/gamemode survival Alex`。当然，`/gamemode s Alex`和`/gamemode 0 Alex`都是可以的。
...
```text title="目标选择器参数的语法" showLineNumbers
m=<游戏模式: int|GameMode>
```
