...
![replaceitem_1](./img/section7/replaceitem_1.png)
...
![replaceitem_2](./img/section7/replaceitem_2.png)
...
![replaceitem_3](./img/section7/replaceitem_3.png)
...
在 [1.6](../chapter1/section6)，我们曾经学过`hasitem`是用来检测玩家是否存在物品的。然而，它并不仅仅是检测是否有物品那么简单。实际上，在 1.18.30 添加这条选择器参数时，基岩版的命令玩家无不为之欢欣雀跃，因为基岩版终于有一个像样的成熟的检测物品的方式了，检测物品的方法也得到了大幅简化，并且拥有更优良的可读性。
...
8. 现在已知`hasitem`可以检测到鼠标在物品栏中拿起来的物品——这类物品不属于任何一种已列出的槽位类型，但是可以被`hasitem`检测得到。当玩家拿起自己头上的钻石头盔时，清除玩家所有的钻石头盔并重新放一个回去。
   ![practice_1](./img/section7/practice_1.png)
