...
本文档对绝大部分正式版或预览版中可用的命令所涉及到的参数类型进行简单介绍。
...
应写入一个 32 位整数（范围从-2,147,483,648~2,147,483,647）。代表该参数为一个数学上的整数。在实际命令中可能存在特定的取值范围。

使用该参数类型的命令一共有 21 条。

<details>

<summary>使用该参数类型的命令</summary>
...
为多条命令可用的穷举参数。如果穷举涉及的可写入内容数目过多则不予一一收录，而直接外链到对应的 Wiki 条目上。
...
仅限于一条命令中可用的穷举参数。如果穷举涉及的可写入内容数目过多则不予一一收录，而直接外链到对应的 Wiki 条目上。
